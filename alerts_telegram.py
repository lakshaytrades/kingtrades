    ...
    def __init__(self, bot_token: str, chat_id: str):
        ...
        if TELEGRAM_AVAILABLE and self.bot_token and self.chat_id:
            try:
                ...
                logger.info(f"[{format_ist_timestamp()}] Telegram alerter initialized successfully with chat_id={self.chat_id}")
            except Exception as e:
                logger.error(f"[{format_ist_timestamp()}] Failed to initialize Telegram Bot: {e}", exc_info=True)
                self._bot = None
        else:
            logger.warning(f"[{format_ist_timestamp()}] Telegram alerter not configured (token or chat_id missing)")

    def _is_ready(self) -> bool:
        if self._bot is None:
            logger.debug("Telegram Bot object is None")
            return False
        if not self.chat_id or self.chat_id == "None":
            logger.debug("Invalid chat_id")
            return False
        return True
    ...
    def _send(
        ...
        try:
            ...
            async def _do_send():
                ...
                logger.info(f"[{format_ist_timestamp()}] Telegram alert sent successfully")

            # Handle both cases: running event loop or no loop
            try:
                loop = asyncio.get_running_loop()
                # Fire-and-forget (non-blocking) — best for background workers
                asyncio.create_task(_do_send())
                logger.debug(f"Telegram alert scheduled asynchronously")
                return True
            except RuntimeError:
