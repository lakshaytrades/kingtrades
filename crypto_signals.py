...
def generate_crypto_signal(symbol: str,
    ...
    if ind1h and ind4h:
        mtf_aligned = (
            (direction == "LONG"  and ind1h.get("ema_trend_up") and ind4h.get("ema_trend_up")) or
            (direction == "SHORT" and not ind1h.get("ema_trend_up", True) and not ind4h.get("ema_trend_up", True))
        )
        if mtf_aligned:
            base_score += 8.0
